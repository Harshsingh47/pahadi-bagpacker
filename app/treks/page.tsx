    ...
    <div className="pt-36 pb-24 bg-[#F8FAF9] min-h-screen">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-xs uppercase tracking-widest font-mono text-emerald-700 font-semibold">HIMALAYAN ALPINE TRAILS</span>
          <h1 className="text-4xl sm:text-6xl font-extrabold text-slate-900 mt-2 uppercase font-heading">FIND YOUR TRAIL</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {treks.map((t) => (
            <div key={t.slug} className="bg-white border border-slate-200/80 rounded-2xl overflow-hidden group hover:border-emerald-500/50 transition-all flex flex-col justify-between shadow-xl">
              <div className="relative h-56">
                ...
                <span className="absolute top-3 left-3 px-2.5 py-1 bg-emerald-50/90 text-emerald-800 text-xs font-bold rounded-lg border border-emerald-200">
                  {t.state} • {t.diff}
                </span>
                <span className="absolute top-3 right-3 px-2.5 py-1 bg-slate-900/80 backdrop-blur-md text-white text-xs font-mono rounded-lg border border-slate-700/50">{t.alt}</span>
              </div>
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-200/80">
                  <div>
                    <span className="text-[10px] text-slate-500 block uppercase">Starting Price</span>
                    <span className="text-lg font-extrabold text-emerald-700">{t.price}</span>
                  </div>
                  <Link href={`/treks/${t.slug}`} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase rounded-xl">
                    View Trail &rarr;
                  </Link>
                </div>
              </div>
