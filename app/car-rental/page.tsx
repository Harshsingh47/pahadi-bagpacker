    ...
    <div className="pt-36 pb-24 bg-slate-950 min-h-screen">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-xs uppercase tracking-widest font-mono text-emerald-400 font-semibold">SELF-DRIVE CAR RENTALS</span>
          <h1 className="text-4xl sm:text-6xl font-extrabold text-white mt-2 uppercase font-heading">CHOOSE YOUR RIDE</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {cars.map((c) => (
            <div key={c.slug} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden group hover:border-emerald-500/50 transition-all flex flex-col justify-between shadow-xl">
              <div className="relative h-48 overflow-hidden">
                <img src={c.img} alt={c.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                <span className="absolute top-3 left-3 px-2.5 py-1 bg-emerald-950 text-emerald-300 text-xs font-bold rounded-lg border border-emerald-800">{c.cat}</span>
              </div>
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                  <div>
                    ...
                    <span className="text-xl font-extrabold text-emerald-400">{c.price}</span> <span className="text-xs text-slate-400">/ day</span>
                  </div>
                  <Link href={`/car-rental/${c.slug}`} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-xl">
                    View Car &rarr;
                  </Link>
                </div>
              </div>
